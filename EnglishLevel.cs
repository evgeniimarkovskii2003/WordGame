...
namespace WordGame
{
    public partial class EnglishLevel : Form
    {
        int fl = 0;
        int fl2 = 0;
        string usedLetters = "";
        ...
        private void ToMainMenu_Click(object sender, EventArgs e)
        {
            ...
            if (dialogResult == DialogResult.Yes)
            {
                ...
                mainMenu.ShowDialog();
                this.FormClosing -= EnglishLevel_FormClosing;
                this.Close();
            }
        }
        ...
        private void EnglishLevel_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void panel1_Paint(object sender, PaintEventArgs e)
        {
            ...
            {
                DataBase database = new DataBase();
                MySqlCommand command = new MySqlCommand("SELECT * FROM `english_level`;", database.getConnection());
                DataTable table = new DataTable();
                MySqlDataAdapter adapter = new MySqlDataAdapter();
                database.openConnection();
                adapter.SelectCommand = command;
                ...
                command = new MySqlCommand("SELECT * FROM `english_level` WHERE `number` = @num", database.getConnection());
            ...
            if ((letter >= 'а' && letter <= 'я' || letter >= 'a' && letter <= 'z') && letters.Text.Length == 1)
            {
                if (usedLetters.IndexOf(letter) == -1)
                {
                    ...
                    if (attemps.Text == "0")
                    {
                        ...
                        computerPlayerWinners.level.Text = "английский";
                        rating = new_rating(1);
