...
namespace WordGame
{
    public partial class LegendLevel : Form
    {
        int fl = 0;
        int fl2 = 0;
        string usedLetters = "";
        ...
        public void decrease_rating()
        {
            ...
            dr.Read();
            int rating = Convert.ToInt32(dr[0]) - 20;;
            dr.Close();
        ...
        {
            DialogResult dialogResult = MessageBox.Show("Вы хотите вернуться в главное меню? Рейтинг будет снижен на 20 очков.", "", MessageBoxButtons.YesNo);
            if (dialogResult == DialogResult.Yes) {
                decrease_rating();
                Hide();
                MainMenu mainMenu = new MainMenu();
                mainMenu.ShowDialog();
                this.FormClosing -= LegendLevel_FormClosing;
                this.Close();
            }
        }
        ...
        private void panel1_Paint(object sender, PaintEventArgs e)
        {
            if (fl2 == 0)
            {
                ...
                command = new MySqlCommand("SELECT * FROM `legend_level` WHERE `number` = @num", database.getConnection());
            ...
            if ((letter >= 'а' && letter <= 'я' || letter >= 'a' && letter <= 'z') && letters.Text.Length == 1)
            {
                if (usedLetters.IndexOf(letter) == -1)
                {
                    ...
                    if (attemps.Text == "0")
                    {
                        ...
                        computerPlayerWinners.level.Text = "легенда";
                        rating = new_rating(1);
        ...
        private void LegendLevel_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (MessageBox.Show("Вы хотите закрыть игру? При закрытии выход из аккаунта будет выполнен автоматически. Рейтинг будет снижен на 20 очков.", "", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.No)
            {
                e.Cancel = true;
            }
            else
